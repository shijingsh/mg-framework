...
/** 
 * GroovyConstants.gc_brackets_mid_end 编译器  
 * 闭合中括号编译器
 * @author: liukefu
 * @date: 2015年4月27日 下午4:33:06  
 */
public class EndMidBracketsCompiler implements CompilerProxy{

	@Override
	public GCompilerResult compile(GSentenceBase sentence, String groovyCode) throws CompilerGroovyException{
		
		int startIndex = CompilerUtil.indexOf(groovyCode,GroovyConstants.gc_brackets_mid_end,0);
		if(startIndex==-1){
		...
		if(matchIndex==-1){
			CompilerUtil.compilerException("语法错误，缺少匹配的字符"+GroovyConstants.gc_brackets_mid_end+"！",groovyCode,CompilerUtil.ERROR_SYSTEM);
		}
		
		return new GCompilerResult(null,GroovyConstants.gc_brackets_mid_end.length());
	}
}
